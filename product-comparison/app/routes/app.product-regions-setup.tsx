...
export default function ProductRegionsSetupPage() {
  ...
  const setupProductRegions = async () => {
    ...
    try {
      ...
      if (response.ok && responseData.success) {
        ...
        setMessage(
          responseData.message ||
            "Failed to set up product regions. Check server logs for details.",
        );
      ...
      setStatus("error");
      setMessage(
        "An unexpected error occurred. Check browser console and server logs.",
      );
  ...
  return (
    ...
    >
      ...
      <p>
        Click the button below to randomly assign a set of available
        regions/sub-regions (from the master list in <code>regions.liquid</code>
        ) to the
        <code>product_specs.available_regions</code> metafield for all products
        in your store.
      </p>
      <p>
        <strong>Important:</strong> This is a one-time setup action for
        development or testing. It will iterate through all products and update
        their metafields. This might take some time for stores with many
        products.
      </p>
