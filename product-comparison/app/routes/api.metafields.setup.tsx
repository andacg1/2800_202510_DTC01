...
export const action: ActionFunction = async ({ request }) => {
    ...
    try {
        ...
        const allProducts = productsData.data.products.edges.map((edge: { node: ProductNode }) => edge.node);
            ...
            try {
                ...
                if (metafields.length > 0) {
                    ...
                    if (responseJson.data?.metafieldsSet?.userErrors && responseJson.data.metafieldsSet.userErrors.length > 0) {
                        results.errors.push(`Product ${product.title}: ${responseJson.data.metafieldsSet.userErrors[0].message}`);
                    } else {
                        results.processed++;
                    }
                }
        ...
        return json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to set up metafields",
